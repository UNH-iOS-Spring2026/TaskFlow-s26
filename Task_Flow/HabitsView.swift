...
struct HabitsView: View {
    ...
    private func normalizeCompletedTodayFlags() {
        ...
        for index in store.habits.indices {
            let shouldBeCompletedToday: Bool

            if let lastCompleted = store.habits[index].lastCompleted {
                shouldBeCompletedToday = Calendar.current.isDateInToday(lastCompleted)
            } else {
                shouldBeCompletedToday = false
            }

            if store.habits[index].isCompletedToday != shouldBeCompletedToday {
                store.habits[index].isCompletedToday = shouldBeCompletedToday
                changed = true
            }
        }
    ...
    private var secondaryTextColor: Color {
        darkMode ? Color.white.opacity(0.72) : Color.black.opacity(0.62)
    }
    ...
    private var accentTileColor: Color {
        darkMode ? Color.orange.opacity(0.85) : Color.orange
    }
